...
function App() {
      ...
      try {
        ...
        if (currentUser) {
          ...
          if (freshUser.length > 0) {
            let updatedPermissions = JSON.parse(freshUser[0].permissions);
            let changed = false;
            if (updatedPermissions.admin && (!updatedPermissions.costReport || !updatedPermissions.costReport.read)) {
                updatedPermissions.costReport = { read: true, write: true };
                changed = true;
            }
            if (changed) {
                await db.execute("UPDATE users SET permissions = $1 WHERE id = $2", [JSON.stringify(updatedPermissions), currentUser.id]);
            }
            const updatedUser = { ...freshUser[0], permissions: updatedPermissions };
  ...
  const navigateTo = (tab, payload = null) => {
    if (tab === 'reports' || tab === 'cost-report') {
      setSelectedReportId(payload);
    } else if (tab === 'containers' && payload && payload.shipmentId) {
  ...
  if (dbError) {
    return (
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh', padding: '20px', color: 'var(--text-color)', backgroundColor: 'var(--bg-color)', fontSize: '18px' }}>
        Can not connect the database, please check if the database is available.
      </div>
    );
  }
      ...
      <div className="topbar">
        <div className="topbar-left">
          ...
          <ul className="nav-menu">
            ...
            {p.costReport?.read && (
              <li className={`nav-item ${activeTab === 'accounting' ? 'active' : ''}`} onClick={() => navigateTo('accounting')}>
                <Calculator size={18} />
                Accounting
              </li>
            )}
        ...
        <div className="content-area">
          ...
          {p.products?.read && (
            <div style={{ display: activeTab === 'products' ? 'block' : 'none', height: '100%' }}>
              <Products currentUser={currentUser} isActive={activeTab === 'products'} />
            </div>
          )}
          {p.inventory?.read && (
          ...
          {p.reports?.read && (
            <div style={{ display: activeTab === 'accounting' ? 'block' : 'none', height: '100%' }}>
              <CostReport currentUser={currentUser} isActive={activeTab === 'accounting'} />
            </div>
          )}
          {(p.containerDashboard?.read || p.shipmentOrders?.read || p.containerList?.read || p.shippers?.read || p.comingProducts?.read || p.comingContainer?.read) && (
            <div style={{ display: activeTab === 'containers' ? 'block' : 'none', height: '100%' }}>
              <Containers currentUser={currentUser} isActive={activeTab === 'containers'} externalShipmentId={selectedShipmentId} onClearExternalShipment={() => setSelectedShipmentId(null)} />
            </div>
          )}
          {p.settings?.read && (
